...
static char const *ldap_escape_query(char const *s, ssize_t cnt_)
{
...
ldap_lookup(config_t agent_config, const char *username, char *external_uid)
{
	...
	BerElement	*ber;
	char		*a, *dn;
	char const	*sane_username = ldap_escape_query(username, -1);
	char		**vals = NULL;
