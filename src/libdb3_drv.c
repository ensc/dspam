/* $Id: libdb3_drv.c,v 1.13 2006/01/20 17:28:33 jonz Exp $ */
...
/*
 * libdb3_drv.c - Berkeley DB v3 storage driver
 *
 * DESCRIPTION
...
int
_libdb3_drv_get_spamtotals (DSPAM_CTX * CTX)
{
  DBT key, data;
  char hashkey[32];
  int ret;
  struct _libdb3_drv_storage *s = (struct _libdb3_drv_storage *) CTX->storage;
  ...
  if (ret)
  {
    ...
    {
      if (!_libdb3_drv_recover (CTX, 1))
      {
        return _libdb3_drv_get_spamtotals (CTX);
      }
      else
      {
        return EUNKNOWN;
...
int
_libdb3_drv_set_spamtotals (DSPAM_CTX * CTX)
{
  DBT key, data;
  char hashkey[32];
  int ret;
  ...
  {
    _libdb3_drv_get_spamtotals (CTX);   /* undo changes to in memory totals */
    return 0;
  }
  ...
  if (ret)
  {
    if (ret == DB_RUNRECOVERY)
    {
      if (!_libdb3_drv_recover (CTX, 1))
      {
        return _libdb3_drv_set_spamtotals (CTX);
      }
      else
      {
        return EUNKNOWN;
...
_ds_get_spamrecord (DSPAM_CTX * CTX, unsigned long long token,
                    struct _ds_spam_stat *stat)
{
  struct _libdb3_drv_spam_record record;
  ...
  if (data.size == sizeof (struct _libdb3_drv_spam_record))
  {
  ...
  {
    LOG (LOG_WARNING,
         "_ds_get_spamrecord: record size (%d) doesn't match sizeof(struct _libdb3_drv_spam_record) (%d)",
         data.size, sizeof (struct _libdb3_drv_spam_record));
    return EUNKNOWN;
  }
...
{
  struct _libdb3_drv_storage *s = NULL;
  ...
  if (!recovery)
  {
    ...
    s = malloc (sizeof (struct _libdb3_drv_storage));
    if (s == NULL)
    {
...
*/
    ...
    lock_result =
      _libdb3_drv_lock_get (CTX, s, (CTX->group ==
  ...
  if (CTX->username != NULL)
  {
    if (_libdb3_drv_get_spamtotals (CTX))
    {
...
_ds_shutdown_storage (DSPAM_CTX * CTX)
{
  ...
  {
    _libdb3_drv_set_spamtotals (CTX);
  }
  ...
  if (s->db)
  {
    ...
    lock_result =
      _libdb3_drv_lock_free (s, (CTX->group ==
                             NULL) ? CTX->username : CTX->group);
    if (lock_result < 0)
    {
      LOGDEBUG ("locking subsystem returned error");
...
_ds_get_nexttoken (DSPAM_CTX * CTX)
{
  ...
  while (s->c->c_get (s->c, &key, &data, DB_NEXT) == 0 && key.size < 64)
  {
    ...
    if (strcmp (keyname, "_TOTALS"))
    {
      ...
      memcpy (&record, data.data, sizeof (struct _libdb3_drv_spam_record));
...
int
_libdb3_drv_lock_get (DSPAM_CTX *CTX, struct _libdb3_drv_storage *s, const char *username)
{
  char filename[MAX_FILENAME_LENGTH];
  int r;
...
int
_libdb3_drv_lock_free (struct _libdb3_drv_storage *s, const char *username)
{
  int r;
...
int
_libdb3_drv_recover (DSPAM_CTX * CTX, int fatal)
{
  struct _libdb3_drv_storage *s = NULL;
  int dbflag = DB_CREATE;
